...
Run to (re)generate the fixture ZIP:

    python tests/_build_fixtures.py

The resulting file is committed to the repo (whitelisted in .gitignore).
"""
...
# File paths inside the ZIP (must match conftest fixture entry).
...
def build_fixture_zip(dest: Path, seed: int = 42) -> None:
    """Generate the synthetic fixture ZIP at `dest`.
