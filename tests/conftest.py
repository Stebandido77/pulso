...
def fixture_zip_path(fixtures_dir: Path) -> Path:
    """Path to the committed synthetic fixture ZIP."""
...
def fixture_zip_sha256(fixture_zip_path: Path) -> str:
    """SHA-256 hex digest of the fixture ZIP."""
    h = hashlib.sha256()
...
def _make_fixture_sources(sha256: str) -> dict[str, Any]:
    """Return a sources dict that mirrors sources.json but adds a 2024-06 entry
    pointing at the synthetic fixture ZIP.
    """
    return {
        "metadata": {
...
def registry_with_fixture(
    ...
    """Inject a synthetic 2024-06 registry entry and pre-populate the cache.

    This fixture:
    1. Redirects PULSO_CACHE_DIR to a temp directory.
    2. Copies the fixture ZIP into the expected cache slot.
    3. Overrides _SOURCES so _load_sources() returns the synthetic entry.
    ...
    # Override the sources singleton with the fixture entry.
    custom_sources = _make_fixture_sources(fixture_zip_sha256)
